...
if [[ ("$TERM" != "dumb") && ("$TERM" != "linux")]]; then
  ...
  local git=214;        local fg_git="$FG[$git]";        local bg_git="$BG[$git]"
  ...
  local venv=105;       local fg_venv="$FG[$venv]";      local bg_venv="$BG[$venv]";
...
function virtualenv_info() { 
 [ $VIRTUAL_ENV ] && echo "$bg_venv$split$fg_text$spad${VIRTUAL_ENV##*/}${RESET}$spad$fg_venv"
}
